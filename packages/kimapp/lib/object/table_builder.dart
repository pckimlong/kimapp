...
class ColumnBuilder with _$ColumnBuilder {
  ...
  String get value {
    return map(
      (value) => value.column,
      join: (value) {
        return '${value.key ?? value.table.tableName}${value.candidateKey.isNullOrBlank ? ":${value.foreignKey}" : ""}${value.candidateKey.isNotNullOrEmpty ? "!${value.candidateKey}" : ""}(${value.table.selectStatement})';
      },
    );
  }
}
